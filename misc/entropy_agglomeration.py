...
def knn_agglomeration( X, counts, p_c, threshold ):
    X_to_clusters = arange( len( X ) )
    over_thresh = ( counts >= threshold )
    new_X = compress( over_thresh, X, 0 )
    new_counts = compress( over_thresh, counts, 0 )
    new_p_c = compress( over_thresh, p_c, 0 )
    new_index = 0
    for i, val in enumerate( over_thresh ):
        if val:
            ...
            dists = sqrt( sum( ( new_X - X[i] )**2, 1 ) )
            closest = argmin( dists )
            sum_p = new_p_c[closest] + p_c[i]
            new_X[closest] = new_X[closest] * ( p_c[closest] / sum_p ) + X[i] * ( p_c[i] / sum_p )
...
def main():
    ...
    # occuring points
    if min_occurences > 1 and not alltrue( counts >= min_occurences ):
        X, counts, p_c, X_to_clusters = knn_agglomeration( X, counts, p_c, min_occurences )
        n_clusters = len( X )
    ...
    while n_clusters > 10:
        ...
        # plot = ( n_clusters > 320 or n_clusters < 30 )
        
        n, m = X.shape
    
        # Indexes of minimum element for each row and column
        ...
        for i in range( n ):
            ...
            if i < j and col_min_indexes[j] == i:
                ...
                cl = arange( n_clusters )
                t = cl[j]
                cl[j] = cl[i]
                for ii in range(n): 
                    if cl[ii] == n_clusters-1: cl[ii] = t
